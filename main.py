...
# --- 基礎設定 ---
VERSION_SN = "v2026.08.22-32"  # 程式版本流水號自動 +1
...
menu_hb = st.sidebar.radio("異常排除手冊導航", [
    ...
    "📊 異常數據統計", 
    "⚙️ 管理後台"
], label_visibility="collapsed", key="radio_hb")

st.sidebar.markdown("---")
st.sidebar.markdown("### 📈 品質異常導航")
menu_ql = st.sidebar.radio("品質異常導航", [
    "📈 異常紀錄查詢"
], label_visibility="collapsed", key="radio_ql")

handbook = load_handbook()
if 'clear_flag' not in st.session_state: st.session_state.clear_flag = 0

# 透過 session_state 追蹤最後點擊的是哪一個選單群組
if st.session_state.get("radio_hb") != st.session_state.get("_last_hb"):
    st.session_state.active_nav_group = "hb"
    st.session_state._last_hb = st.session_state.get("radio_hb")

if st.session_state.get("radio_ql") != st.session_state.get("_last_ql"):
    st.session_state.active_nav_group = "ql"
    st.session_state._last_ql = st.session_state.get("radio_ql")

# --- 路由分發邏輯 ---
if st.session_state.active_nav_group == "ql":
    try:
        import main2
        main2.render_page()
    except Exception as e:
